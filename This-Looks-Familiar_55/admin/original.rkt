...
(define FLAG "stuyctf{racket_i$_pr3tty_kewl}")
...
(define (decrypt-flag cipher)
  (if (= (length cipher) 0)
      (list)
      (if (equal? (car (check-numbers-are-prime cipher)) #f);#t) ; Uncomment for the correct decryption. This will have to be a fix 
...
(define encrypted-flag (encrypt-flag FLAG))
encrypted-flag ; Prints out the encrypted flag
(list->string (decrypt-flag encrypted-flag))
(list->string (decrypt-flag (list 115 116 117 121 99 116 102 123 114 97 98 99 107 108 101 102 116 95 105 36 95 112 114 51 116 116 121 95 107 108 101 102 119 108 125)))
